...
// Model for Player entity
    models.Player = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/PlayerREST/api/player/",
        idAttribute: 'id',
        defaults: {
            firstname: "",
            jerseynumber: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/PlayerREST/api/player/';
            }
    ...
    // Collection class for Player entities
    models.PlayerCollection = Backbone.Collection.extend({
        model: models.Player,
        url: "http://localhost:8080/PlayerREST/api/player/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.PlayerCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-player-list-itemis template identifier for item
                        templateName: '#tpl-player-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.player = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.player,
                    // tpl-player-details is template identifier for chosen model element
                    templateName: '#tpl-player-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Player();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-player-details is a template identifier for chosen model element
                templateName: '#tpl-player-details',
        ...
        getData: function () {
            return {
                ...
                jerseynumber: $('#jerseynumber').val(),
                lastspokenwords: $('#lastspokenwords').val(),
