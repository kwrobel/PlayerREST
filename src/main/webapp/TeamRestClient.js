...
// Model for Team entity
    models.Team = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/PlayerREST/api/team/",
        idAttribute: 'teamId',
        defaults: {
            league: "",
            teamname: ""
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('teamId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/PlayerREST/api/team/';
            }
    ...
    // Collection class for Team entities
    models.TeamCollection = Backbone.Collection.extend({
        model: models.Team,
        url: "http://localhost:8080/PlayerREST/api/team/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.TeamCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-team-list-itemis template identifier for item
                        templateName: '#tpl-team-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.team = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.team,
                    // tpl-team-details is template identifier for chosen model element
                    templateName: '#tpl-team-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Team();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-team-details is a template identifier for chosen model element
                templateName: '#tpl-team-details',
